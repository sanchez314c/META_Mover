...
####################################################################################
#
# Script Name: media-organizer-audio.py                                        
# 
...
# Version: 1.7.0                                                                 
#
# Description: Audio-focused media organizer optimized for music and audio files    
#              with advanced subsecond detection, year-based organization, and  
#              multi-core processing with final sweep capability.                                              
#
# Usage: python media-organizer-audio.py [--input PATH] [--output DIR]
#
# Dependencies: exiftool, mutagen, tqdm, tkinter                                           
#
# GitHub: https://github.com/sanchez314c                                         
#
# Notes: Specialized for audio formats with enhanced metadata extraction,      
#        subsecond precision with -SS formatting, and intelligent error  
#        recovery with small files handling.                                                    
#                                                                                
####################################################################################

"""
Audio Media Organizer - Final Sweep Edition
==========================================

Audio-focused organizer optimized for music and audio files with subsecond precision,
year-based organization, and comprehensive metadata handling with final sweep capability.
"""
    ...
    if 'ExifIFD' in metadata and isinstance(metadata['ExifIFD'], dict):
        ...
        for field in subsec_fields:
            if field in exif_ifd and exif_ifd[field]:
                ...
                if subsec.isdigit() and int(subsec) > 0:
                    return subsec.zfill(6)[:6], False
...
def determine_target_directory(destination_root: str, media_type: str, date: datetime, is_error: bool = False) -> str:
    year = date.strftime('%Y')
    
    if is_error:
        return os.path.join(destination_root, 'Error', 'Audio', year)
    else:
        return os.path.join(destination_root, 'Audio', year)
...
def process_file(args: Tuple[str, str, str, bool]) -> Tuple[bool, str]:
    ...
    try:
        ...
        new_filename = generate_filename_with_ss(date, subseconds, os.path.splitext(file_path)[1])
        
        target_directory = determine_target_directory(destination_root, media_type, date, is_error_sweep)
...
def main() -> None:
    ...
    print("Audio Media Organizer - Final Sweep Edition")
    print("----------------------------------------------------------")
    ...
    def update_scanning_progress():
        ...
        if current_time - last_update_time >= 0.5:
            sys.stdout.write(f"\rScanning... {scanning_progress[scanning_idx]} Found {files_found} audio files so far")
            sys.stdout.flush()
    ...
    sys.stdout.write(f"\rScanned {len(files)} audio files in total.{' ' * 30}\n")
    sys.stdout.flush()
    
    total_files = len(files)
    
    if total_files == 0:
        print("No supported audio files found in the source directory.")
        ...
        print(f"Found {total_files} audio files in source directory.")
